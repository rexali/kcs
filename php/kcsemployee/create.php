...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($address_err) && empty($salary_err) && empty($class_err) && empty($phone_err) && empty($discipline_err) && empty($appointment_date_err)){
        ...
        $sql = "INSERT INTO kcs_employees (name, address, salary, class, phone, discipline, appointment_date, subject ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ? )";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssss", $param_name, $param_address, $param_salary, $param_class, $param_phone, $param_discipline, $param_appointment_date, $param_subject);
